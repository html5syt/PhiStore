...
var ShopAvatar = preload("res://components/v1/ShopAvatar.tscn")
var songIDs = {}
    ...
    if pageType != page_type.Avatar:
        ...
        for avatar in avatars[1]:
            ...
            songs_to_load.push_back({
                "itemName": avatar,
                "data": "%.2f MB" % data,
                "dataOff": "%.2f MB" % (int(data * dataOffPrecentRand * 100) / 100.0) if avatar in offAvatars else "",
                "dataOffPrecent": dataOffPrecentRand if avatar in offAvatars else 0.0,
                "illustration": illustration,
                "pageType": pageType,
                "isSoldOut": false
            })
...
func _create_song_item(song_data: Dictionary) -> void:
    var songContainer
    if pageType != page_type.Avatar:
        songContainer = ShopSong.instantiate()
    else:
        songContainer = ShopAvatar.instantiate()
    ...
    songContainer.pageType = song_data["pageType"]
